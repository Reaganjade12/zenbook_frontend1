...
// Get massage duration in minutes
function getMassageDuration(serviceType) {
    const durations = {
        'Swedish Massage': 60,
    ...
    return durations[serviceType] || 60; // Default to 60 minutes
}

// Format massage type with duration
function formatMassageType(serviceType) {
    if (!serviceType) return '';
    const duration = getMassageDuration(serviceType);
    // Format as "Massage Type (X minutes)" for clarity
    if (duration === 60) {
        return `${serviceType} (60 minutes)`;
    } else if (duration === 90) {
        return `${serviceType} (90 minutes)`;
    } else if (duration === 75) {
        return `${serviceType} (75 minutes)`;
    } else {
        return `${serviceType} (${duration} minutes)`;
    }
}
...
function ensureAdminLayoutStyles() {
    ...
    style.textContent = `
        .admin-sidebar {
            ...
            max-height: 100vh;
            max-height: -webkit-fill-available; /* iOS fix */
            overflow: hidden;
            z-index: 1000;
...
window.formatMassageType = formatMassageType;
window.getMassageDuration = getMassageDuration;
